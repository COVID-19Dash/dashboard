...
for line in datarows:
	...
	if (int(recover) > 0):
		if (prov != ""):
			marker = "L.circle([" + lat + "," + lon + "],{color:'green',fillColor:'#3eec38',fillOpacity:0.5,radius:" + str(recoverradius) + "}).addTo(group2).bindPopup('" + prov.replace("'", "") + "," + country.replace("'","") + " : " + recover + "')"	
		else:
			marker = "L.circle([" + lat + "," + lon + "],{color:'green',fillColor:'#3eec38',fillOpacity:0.5,radius:" + str(recoverradius) + "}).addTo(group2).bindPopup('" + country.replace("'", "") + " : " + recover + "')"
